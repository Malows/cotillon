...
class Arqueos extends CI_Controller {
  ...
  public function estimar_caja () {
    if (count($this->caja->lista_cajas_abiertas()) != 1)
      return json_encode(['success' => false, 'message' => 'No existen cajas abiertas.']);
    return json_encode(['success' => true, 'caja' => $this->caja->estimar_caja()]);
  }
  ...
  public function cerrar_caja () {
    ...
    if (count($this->caja->lista_cajas_abiertas()) != 1)
      return json_encode(['success' => false, 'message' => 'No existen cajas abiertas.']);
    return json_encode(['success' => true, 'caja' => $this->caja->cerrar_caja($real)]);
  }

  public function abrir_caja () {
    $monto = floatval( $this->input->post('monto') );
    if (count($this->caja->lista_cajas_abiertas()) != 0)
      return json_encode(['success' => false, 'message' => 'Existen cajas abiertas.']);
    return json_encode(['success' => true, 'caja' => $this->caja->abrir_caja($monto)]);
  }
